...
int main()
{
    ...
    LOG_PRINT_INFO("ipc zmq app1 is running!");

    reg_info.module_id = E_IPC_ZMQ_MODULE_ID_TEST1;
    reg_info.work_mode = E_IPC_ZMQ_WORKMODE_SINGLE_WORKER;
    strncpy(reg_info.module_name, "ipc_zmq_app1", sizeof(reg_info.module_name));
    ...
    int notify = 1;
    ret = ipc_zmq_send_notify(E_IPC_ZMQ_MODULE_ID_TEST1, E_IPC_ZMQ_MODULE_ID_MANAGER, E_IPC_ZMQ_MSG_ID_TEST_NOTIFY,
                              (uint8_t *)&notify, sizeof(int));
    if (0 != ret)
    {
        LOG_PRINT_ERROR("ipc_zmq_send_notify fail, ret[%d]!", ret);
        return ret;
    }
    ...
    ret =
        ipc_zmq_send_sync(E_IPC_ZMQ_MODULE_ID_TEST1, E_IPC_ZMQ_MODULE_ID_MANAGER, E_IPC_ZMQ_MSG_ID_TEST_SYNC,
                          (uint8_t *)&sync_req, sizeof(int), (uint8_t *)&sync_resp, &sync_resp_len, sizeof(int), 3000);
    if (0 != ret)
    {
        LOG_PRINT_ERROR("ipc_zmq_send_sync fail, ret[%d]!", ret);
        return ret;
    }
    LOG_PRINT_INFO("recv sync_resp_len[%zd], sync_resp[%d]", sync_resp_len, sync_resp);

    int async_req = 3;
    ret = ipc_zmq_send_async(E_IPC_ZMQ_MODULE_ID_TEST1, E_IPC_ZMQ_MODULE_ID_MANAGER, E_IPC_ZMQ_MSG_ID_TEST_ASYNC,
                             (uint8_t *)&async_req, sizeof(async_req), sizeof(int));
    if (0 != ret)
    {
        LOG_PRINT_ERROR("ipc_zmq_send_async fail, ret[%d]!", ret);
        return ret;
    }

    int broadcast = 4;
    ret = ipc_zmq_send_broadcast(E_IPC_ZMQ_MODULE_ID_TEST1, E_IPC_ZMQ_MSG_ID_TEST_BROADCAST, (uint8_t *)&broadcast,
                                 sizeof(int));
    if (0 != ret)
    {
        LOG_PRINT_ERROR("ipc_zmq_send_broadcast fail, ret[%d]!", ret);
        return ret;
    }

    int stream = 5;
    ret = ipc_zmq_send_stream(E_IPC_ZMQ_MODULE_ID_TEST1, E_IPC_ZMQ_MODULE_ID_MANAGER, E_IPC_ZMQ_MSG_ID_TEST_STREAM,
                              (uint8_t *)&stream, sizeof(int), 10);
    if (0 != ret)
    {
        LOG_PRINT_ERROR("ipc_zmq_send_stream fail, ret[%d]!", ret);
    }

    LOG_PRINT_INFO("test end");
    ...
    ipc_zmq_destroy(E_IPC_ZMQ_MODULE_ID_TEST1);
